# O'Blog — Application de blog avec Express & EJS  

Projet réalisé dans le cadre d’un exercice de formation, visant à concevoir une application de blog côté serveur en JavaScript.

L’objectif de ce projet est de mettre en place une application web simple permettant :
- d’afficher une liste d’articles,
- de naviguer vers le détail d’un article,
- et de structurer proprement une application Express avec des vues dynamiques.

Le projet met l’accent sur la compréhension des bases du **back-end JavaScript**, du **routage**, et du **templating** côté serveur.

---

## Compétences mobilisées

- Mise en place d’un serveur **Node.js / Express**
- Organisation d’un projet back-end (routes, contrôleurs, données)
- Rendu de vues dynamiques avec **EJS**
- Utilisation de **partials** pour factoriser les vues
- Routage statique et routage paramétré
- Manipulation de données (tableaux, objets, filtrage)
- Gestion des paramètres d’URL et des requêtes utilisateur
- Séparation des responsabilités (logique / vues / données)

---

## Fonctionnalités principales

- Page d’accueil listant les articles de manière dynamique
- Page de détail pour chaque article via une route paramétrée
- Navigation entre les pages
- Structure de vues factorisée et maintenable

---

## What I learned — Ce que j’ai appris

- Comprendre le fonctionnement d’un serveur **Express** et le cycle requête / réponse
- Mettre en place un routage clair et lisible, y compris avec des paramètres dynamiques
- Passer des données du serveur vers les vues et les exploiter côté template
- Factoriser le code HTML avec des **partials EJS** pour éviter les duplications
- Structurer un projet back-end de façon plus maintenable
- Manipuler des données côté serveur pour répondre à des besoins d’affichage
- Déboguer des erreurs courantes liées au rendu des vues et aux routes

---

## Tech Stack

- **Node.js**
- **Express**
- **EJS**

---

---

# O'Blog — Blog application with Express & EJS  

Project developed as part of a training exercise, focused on building a server-side blog application using JavaScript.

The goal of this project is to create a simple web application that:
- displays a list of blog articles,
- allows navigation to an article detail page,
- and follows a clean and structured Express architecture.

This project focuses on learning **JavaScript back-end fundamentals**, **routing**, and **server-side templating**.

---

## Skills & Concepts Used

- Setting up a **Node.js / Express** server
- Back-end project structure (routes, controllers, data)
- Dynamic rendering with **EJS**
- View factorization using **partials**
- Static and dynamic routing
- Data manipulation (arrays, objects, filtering)
- URL parameters and request handling
- Separation of concerns (logic / views / data)

---

## Main Features

- Dynamic homepage displaying blog articles
- Article detail pages using parameterized routes
- Navigation between views
- Maintainable and reusable view structure

---

## What I learned

- How an **Express** server handles requests and responses
- How to implement clean and readable routing, including dynamic routes
- Passing data from controllers to views and rendering it with templates
- Using **EJS partials** to avoid duplicated markup
- Structuring a small back-end project for better maintainability
- Manipulating server-side data to drive dynamic content
- Debugging common issues related to routing and template rendering

---

## Tech Stack

- **Node.js**
- **Express**
- **EJS**
